...
getWeekReport = function getWeekReport(weeks, totalTime) {
  ...
  var needTime = getWeekNeedTime(countableDaysOfWeek);
  totalTime += weekTime - needTime;
...
getMonthData = function getMonthData(month, done) {
  ...
  search.searchDaysFromWeek(0, month, processWeek, null);
};
...
buildMonthReport = function buildMonthReport(month, monthWeeks) {
  ...
  monthWeeks.reverse().forEach(function (w) {
    monthReport += getWeekReport(w, totalTime);
  });
